...
contract LockedStaking{
    ...
    //Always review lock type details before deploy
    //Lock Types:
    //6M 10% 
    //11M 22.5% 
    ...
    function CreateLock(uint8 Type, uint256 amount) public returns(bool success){
        require(amount >= 1000000000000000000, "The minimum deposit for staking is 1 TKN");
        require(ERC20(TKN).transferFrom(msg.sender, address(this), amount), "Unable to transfer TKN to the contract, may be due to low allowance or not enough funds");
        ...
        require(GetTKNAvailable() >= (AmountOnWithdraw - amount), "Insufficient TKN in the contract to create lock");
    ...
    function ClaimLock(uint256 ID) public returns(bool success){
        ...
        TotalDeposits -= UserLocks[msg.sender][ID].DepositAmount;
        TotalToBeWithdrawn -= amount;
        ...
        ERC20(TKN).transfer(msg.sender, amount);
        return(success);
    }
