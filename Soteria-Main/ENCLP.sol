...
//        ERC721(CollectionAddress).safeTransferFrom(msg.sender, address(this), ID);
    ...
    function InitializeCounterOffer(uint256 LoanID, bool OfferedTerm, uint256 COterm, bool ChangeInterest, uint256 COir, bool ChangeETH, uint256 COeth) returns(uint256 COID){
        require(UIDmapping[UID].Active == true);
        require(UIDmapping[UID].InLoan == false);
